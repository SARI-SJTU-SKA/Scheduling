...
def main():
	...
	parser.add_argument('-v', '--numV', type=parsePositiveInteger, help='the number of vertices', default=20000)

	parser.add_argument('-e', '--numE', type=parsePositiveInteger, help='the number of edge', default=10000)

	parser.add_argument('-l', '--lenR', type=parseRange, help='the parseRange of the subtask instruction length',
						default='1200,20000')
	...
	ft = codecs.open('./sh.txt', 'w', 'utf-8')
	ft.write(prettyroot)
	# writer.close()
	# os.mknod('sh.txt')

	# try:
	# 	with open('./sh.xml', 'w+', encoding='UTF-8') as fh:
