...
public class TaskBrokerGA extends DatacenterBroker {
	...
	public TaskBrokerGA(String name) throws Exception {
		super(name);
	}
	...
	private Map<Subtask, Vm> scheduling() {
		...
		for (Subtask st : runnableCloudlets) {
			int vmId = map.get(st.getRef());
			_map.put(st, VmList.getById(vmList, vmId));
		}
		return _map;
	}
